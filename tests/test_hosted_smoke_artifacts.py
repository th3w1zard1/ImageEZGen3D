...
class HostedSmokeArtifactsTests(unittest.TestCase):
    ...
    def test_verify_hosted_smoke_artifacts_cli_fails_on_invalid_golden(
        self,
    ) -> None:
        balanced = HostedGoldenSmokeResult(
            ok=True,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            tier_path.write_text(
                json.dumps(
                    {"checks": [balanced.to_dict(), balanced.to_dict()]},
                    indent=2,
                ),
                encoding="utf-8",
            )
        ...
        self.assertIn("g7_false_neural_guard_ok", proc.stderr)
