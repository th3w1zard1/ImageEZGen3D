...
class HunyuanG9EnablementEvidenceBundleTests(unittest.TestCase):
    ...
    @mock.patch(
        "imageezgen3d.hunyuan_g9_enablement_evidence_bundle.run_g9_enablement_evidence_bundle"
    )
    def test_strict_script_fails_when_parity_not_ok(
        ...
        with tempfile.TemporaryDirectory() as directory:
            record_dir = Path(directory)
            record_path = record_dir / "g9-enablement-evidence.json"
            record_path.write_text("{}", encoding="utf-8")
            neural = _neural_result(directory=record_dir, ready=True)
            bundle_fn.return_value = G9EnablementEvidenceBundleResult(
            ...
            script_path = repo_root / "scripts" / "hunyuan_g9_enablement_evidence_bundle.py"
            spec = importlib.util.spec_from_file_location(
                "hunyuan_g9_enablement_evidence_bundle_cli",
                script_path,
            )
    ...
    def test_verify_files_passes_after_ci_like_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            record_dir = Path(directory)
            run_g9_enablement_evidence_bundle(record_dir=record_dir, skip_weight_warm=True)
            issues = verify_g9_enablement_evidence_bundle_files(record_dir)
            self.assertEqual(issues, [])

    def test_verify_files_fails_when_evidence_missing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            issues = verify_g9_enablement_evidence_bundle_files(Path(directory))
            self.assertTrue(any("missing file" in issue for issue in issues))

    def test_verify_g9_enablement_evidence_bundle_script(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            record_dir = Path(directory)
            run_g9_enablement_evidence_bundle(record_dir=record_dir, skip_weight_warm=True)
            result = subprocess.run(
                [
                    sys.executable,
                    "scripts/verify_g9_enablement_evidence_bundle.py",
                    "--record-dir",
                    str(record_dir),
                ],
                check=False,
