...
from imageezgen3d.hunyuan_admission_g9_enablement_evidence_bundle import (  # noqa: E402
    admission_g9_enablement_evidence_bundle_json,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "in one step (does not enable the adapter)."
        )
    )
    parser.add_argument(
        "--record-dir",
        type=Path,
        default=Path("."),
        help="Directory for admission, enablement, and G9 evidence JSON records.",
    )
    parser.add_argument(
        "--skip-weight-warm",
        action="store_true",
    ...
    if args.as_json:
        print(admission_g9_enablement_evidence_bundle_json(result), end="")
    else:
        print(format_admission_g9_enablement_evidence_bundle_report(result), end="")

    if args.strict and not result.g9_enablement_evidence_ready:
        return 1
    if args.strict and not result.parity_ok:
        return 1
    if not result.admission_g9_enablement_evidence_ok:
        return 1
    return 0
