...
from imageezgen3d.hunyuan_enablement_evidence_capstones import (  # noqa: E402
    enablement_evidence_capstones_json,
    format_enablement_evidence_capstones_report,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run admission capstone plus umbrella capstone verify in one step "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("."),
        help="Directory for enablement capstone JSON records.",
    )
    parser.add_argument(
        "--skip-weight-warm",
        action="store_true",
    ...
    if args.as_json:
        print(enablement_evidence_capstones_json(result), end="")
    else:
        print(format_enablement_evidence_capstones_report(result), end="")

    if args.strict and not result.g9_enablement_evidence_ready:
        return 1
    if args.strict and not result.parity_ok:
        return 1
    if not result.enablement_evidence_capstones_ok:
        return 1
    return 0
