...
| P6 | Requirements vs pyproject | `requirements.txt` vs `[project.optional-dependencies]` | 2026-06-03 | `[REPO]` Space `requirements.txt` now includes `usd-core>=24.0` (mesh-delivery parity); audit in `space-dependency-audit.md` | Review on dep PRs | OK |
...
| P12 | Golden sample CI attestation | `scripts/golden_sample_attestation.py` + `.github/workflows/ci.yml` job `golden-sample` | 2026-06-03 | `[REPO]` Block PNG → cpu-demo; manifest/GLB/OBJ/**FBX**/**USDZ†**/export_sidecar gates; †USDZ when usd-core installed | Does not replace hosted P10 E2E or ZeroGPU verification | OK |
