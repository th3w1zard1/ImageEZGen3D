...
**CI:** `hunyuan-admission-audit` runs local G1–G6 readiness. Scheduled `hosted-golden-smoke` also runs `--live-probe --record hunyuan-g7-live-probe.json` to prove the live Space does not report false G7 neural success while the adapter is disabled. Admission audit JSON is built by `imageezgen3d.hunyuan_admission_audit.build_admission_audit_payload()` and includes `g7_readiness` / `g8_enablement`; exits 1 if G1–G6 regress. Hosted doc path: `hosted_validation.HOSTED_VALIDATION_PATH`.
